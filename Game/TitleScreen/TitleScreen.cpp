...
#include ".\titlescreen_text.cpp"//to prevent visual studio make crash charater
	...
	// This function will be triggered every single frames (probably) 
	int CTitleScreen::Step()
	{
		...
		// if Tama didn't rolled completely
		if (m_flagRollingTama != 2)
		{
			//Initialize tama if there's no tama yet
			if (m_flagRollingTama == 0)
			{
				m_tama.Initialize();
				m_flagRollingTama = 1;
			}
			//if tama's current step is 1 (a.k.a. rolling)
			if (m_tama.Step() == 1)
			{
				//tama has been fully rolled (more rolls than a bakery LOL)
				m_flagRollingTama = 2;
				//Complete the tama rolling procedure and trigger Main Menu
				OnRollTamaEnd();
				if (m_bInitWithMusic)
				{
					CCommonFunctionMusicSE::LoadMusicFromDat(&CGame::s_pCurGame->m_th5Dat1, "OP");
		...
		// if fade value is under 0, add 6 on every step.
		if (m_curScrFade < 100)
		{
			m_curScrFade += 6;
			if (m_curScrFade > 100)
	...
	int CTitleScreen::CheckMenuColor(int i) {
		int itemColorIdx = MENU_ITEM_COLOR_NORMAL;
		switch (m_curMenu) {
			//Main Menu
		case 0:
			//if the menu is current cursor position
			if (i == m_mainMenuCursorPosition)
				//highlight it
				itemColorIdx = MENU_ITEM_COLOR_HIGHLIGHTED;
			// if the menu item is not enabled
			if (m_bMainMenuItemEnabled[i] == false)
				// set menu item color as disabled
	...
	float CTitleScreen::GetMenuItemColor(int itemColorIdx, int i) {
		float menuItemColor = m_globalPalette[itemColorIdx * 3 + i] / 255.0f;
		return menuItemColor;
	}

	void CTitleScreen::DrawMainMenuItems()
	{
		// for every single menu in main menu
		for (int i = 0; i < m_mainMenuNItem; i++)
		{
			//capsulize even more
			int itemColorIdx = CheckMenuColor(i);
			float menuItemColor[3];
			for (int j = 0; j < 3; j++) {
				menuItemColor[j] = GetMenuItemColor(itemColorIdx, j);
			}
			
			//if the menu is replay
	...
	{
		// for every single item on option menu
		for (int i = 0; i < m_optionMenuNItem; i++)
		{
			//capsulized even more - Multiple declaration since warning C4700
			int itemColorIdx = 0;
			float menuItemColor[3] = { 0,0,0 };
			itemColorIdx = CheckMenuColor(i);
			for (int j = 0; j < 3; j++) {
				menuItemColor[j] = GetMenuItemColor(itemColorIdx, j);
			}
	...
	void CTitleScreen::ParseKeyEvent()
	{
		if (m_bMenuOn)
		{
			...
			// if, Current Menu is Options Menu
			if (m_curMenu == 1)
			{
				unsigned short curKeyState = CCommonFunctionInput::GetAllKeyState();
				if (CCommonFunctionInput::UpPressed(curKeyState, m_lastKeyState))
