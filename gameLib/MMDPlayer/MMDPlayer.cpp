...
LPDIRECTSOUND8 CMMDPlayer::s_pDSound8=NULL;
LPDIRECTSOUNDBUFFER8 CMMDPlayer::s_pDSoundBuffer8=NULL;
HANDLE CMMDPlayer::s_hEvent[s_bufNSeg+1];
bool CMMDPlayer::s_bMusicDataExist=false;
DWORD CMMDPlayer::s_playThreadID=0;
HANDLE CMMDPlayer::s_hPlayThread=NULL;
bool CMMDPlayer::s_bPlayingWhenLastDeactivate=false;
bool CMMDPlayer::s_bInitialized=false;
int CMMDPlayer::s_localBufferReadHead;
int CMMDPlayer::s_localBufferWriteHead;
unsigned char CMMDPlayer::s_localBuffer[s_localBufferLength];
CRITICAL_SECTION CMMDPlayer::s_csLocalBuf;
...
LPDIRECTSOUNDBUFFER8 CMMDPlayer::CreateBasicBuffer() 
{ 
...
bool CMMDPlayer::LoadMMDData(unsigned char *musicData,int size)
{
	if (s_bMusicDataExist)
	{
		Pause();
		if (UnloadMMDData()==false)
			return false;
...
void CMMDPlayer::SetPlayThreadPriority(int nPriority)
{
	if (s_hPlayThread!=NULL)
		SetThreadPriority(s_hPlayThread,nPriority);
}

bool CMMDPlayer::FillSoftwareBuffer(int nSample)
{
	if (s_bMusicDataExist==false)
		return false;
...
bool CMMDPlayer::CopyFromSoftwareBuffer(unsigned char *outBuf,int copyLen)
{
	EnterCriticalSection(&s_csLocalBuf);
...
DWORD CMMDPlayer::PlayThread(void *pParam)
{
	bool firstTime=true;
	while(1)
	{
...
void CMMDPlayer::SetVolume(float volume)
{
	if (volume>100.0f)
		volume=100.0f;
	if (volume<0.0f)
